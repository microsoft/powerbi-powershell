...
namespace Microsoft.PowerBI.Commands.Workspaces
{
    ...
    public class RestorePowerBIWorkspace : PowerBIClientCmdlet, IUserScope
    {
        ...
        protected override void ExecuteCmdlet()
        {
            ...
            var groupRestoreRequest = new Group { Name = this.Name, Description = this.EmailAddress }; //update type to GroupRestoreRequest

            if (this.ParameterSetName.Equals(PropertiesParameterSetName))
            {
                var response = client.Groups.UpdateGroupAsAdmin(this.Id.ToString(), groupRestoreRequest); //change method to RestoreDeletedGroupAsAdmin
                this.Logger.WriteObject(response);
            }
            else if (this.ParameterSetName.Equals(WorkspaceParameterSetName))
            {
                var response = client.Groups.UpdateGroupAsAdmin(this.Workspace.Id.ToString(), groupRestoreRequest); //change method to RestoreDeletedGroupAsAdmin
                this.Logger.WriteObject(response);
            }
        }
    }
